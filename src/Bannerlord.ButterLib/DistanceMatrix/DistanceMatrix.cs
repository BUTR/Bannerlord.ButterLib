...
namespace Bannerlord.ButterLib.DistanceMatrix
{
    ...
    /// For any other <see cref="MBObjectBase" /> subtypes custom EntityListGetter and DistanceCalculator
    ...
    public sealed class DistanceMatrix<T> : ISerializable where T : MBObjectBase
    {
        ...
        /// and distances between them as value.
        /// </summary>
        public Dictionary<ulong, float> AsDictionary => _DistanceMatrix;

        /// <summary>A dictionary of paired type <typeparamref name="T"/> objects as key and distances between them as value.</summary>
        public Dictionary<(T object1, T object2), float> AsTypedDictionary => _TypedDistanceMatrix;

        //Constructors
        /// <summary>Initializes a new instance of the <see cref="T:Bannerlord.ButterLib.DistanceMatrix.DistanceMatrix`1" /> class.</summary>
        ...
        /// with custom calculation method to be used.
        /// </summary>
        /// <param name="customListGetter">
        /// A method delegate to be used to get a list of type <typeparamref name="T"/> objects
        /// to calculate the distance.
        /// </param>
        /// <param name="customDistanceCalculator">
        /// A method delegate to be used to calculate the distance between two given type <typeparamref name="T"/> objects.
        /// </param>
        /// <exception cref="T:System.ArgumentException"></exception>
        ...
        /// <summary>Initializes a new instance of the <see cref="T:Bannerlord.ButterLib.DistanceMatrix.DistanceMatrix`1" /> class.</summary>
        /// <remarks>Used for the deserialization.</remarks>
        ...
        /// or <see cref="float.NaN" /> if distance was not or could not be calculated.
        /// </returns>
        public float GetDistance(T object1, T object2)
        {
            return _DistanceMatrix.TryGetValue(ElegantPairHelper.Pair(object1.Id, object2.Id), out float distance) ? distance : float.NaN;
        }

        /// <summary>Calculates distance between two given <see cref="Hero" /> objects.</summary>
        /// <param name="hero1">First of the heroes to calculate distance between.</param>
        /// <param name="hero2">Second of the heroes to calculate distance between.</param>
        /// <returns>
        ...
        /// <param name="settlementOwnerPairedList">
        /// List of the distances between pairs of settlements and of the weights of the paired settlements,
        /// except that the owner clan pairs are used instead of the settlements themselves.
        /// </param>
        /// <returns>
        ...
        public static float CalculateDistanceBetweenClans(Clan clan1, Clan clan2, List<(ulong owners, float distance, float weight)> settlementOwnerPairedList)
        {
            ulong pair = clan1.Id > clan2.Id ? ElegantPairHelper.Pair(clan2.Id, clan1.Id) : ElegantPairHelper.Pair(clan1.Id, clan2.Id);
            List<(float distance, float weight)> settlementDistances = settlementOwnerPairedList.Where(tuple => tuple.owners == pair && !float.IsNaN(tuple.distance))
        ...
        private static float CalculateDistanceBetweenKingdoms(Kingdom kingdom1, Kingdom kingdom2, DistanceMatrix<Settlement> settlementDistanceMatrix)
        {
            ...
            (float distance, float weight) WeightedSettlementSelector(KeyValuePair<(Settlement object1, Settlement object2), float> x) =>
        ...
        {
            return settlementDistances.Any(x => x.weight > 0)
        ...
        {
            return _DistanceMatrix.ToDictionary(key => ElegantPairHelper.UnPairMBGUID(key.Key), value => value.Value)
